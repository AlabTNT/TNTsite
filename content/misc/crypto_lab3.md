...
题目给了一个.sage文件，实际就是sage环境下的python。看了一下内容，前面是创建了一个在Fp上的椭圆曲线y^2=x^3+7，然后指定了一个点G。后面开始签名，签名18次给出18次的结果。
...
因为 $ s_i $ 是通过 $ k_i^-1 $ 生成的，所以有模逆，转化一下刚才的式子就能得到：
$$ k\_i \\equiv (s\_i^{-1} r\_i) \\cdot Flag + (s\_i^{-1} h) \\pmod n $$

这个式子变得舒爽很多，因为只有 $ k_i $ 和 $ Flag $ 被拿出来了，其他的都是他们的系数。换元法设
...
0 & 0 & 0 & \cdots & n & 0 & 0 \newline
u_1 & u_2 & u_3 & \cdots & u_{18} & C_1 & 0 \newline
v_1 & v_2 & v_3 & \cdots & v_{18} & 0 & C_2
\end{pmatrix}
$$
...
本题中p和q大致相近，$ p \apporx N^{\frac{1}{2}} $ 成立，可以使用Coppersmith进行
...
可以得到一个多项式
$$ f(x) = x^2 - sx + v \pmod N $$
其中 $ m $ 是该多项式的一个根。Sagemath内置了small_roots，因此可以直接使用。写出如下脚本：
