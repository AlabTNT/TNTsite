...
这里分别进行 `gcc -E hello.c -o hello.c.i` 、 `gcc -S hello.c`
、`gcc -c hello.c`
、`gcc hello.c -o hello`，可以分别得到hello.c.i、hello.s、hello.o和hello.exe。

其中.i是预处理文件，里面堆了一堆include内容和一小点点的int
main，main里面也没有进行处理。
...
（后期补充一下：想起来什么时候用流程图了，当时和北京一哥们一块爆classin，想实现一下防撤回，搞忘记是做什么了，他最开始拿到的内容是纯汇编，就是那一大坨一大坨的三元运算，在jne的海洋中徜徉了不知道多久，后来改用了流程图稍微清晰了点，最后把"小A
撤回了一条消息"给魔改成了"小A"，这个用户名逻辑说啥也没找到。这也是第一次汇编给我留下的心理阴影）
...
#figure(image("image-16.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-17.png", alt: "alt text"),
  caption: [
    alt text
  ]
)

#figure(image("image-18.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-19.png", alt: "alt text"),
  caption: [
    alt text
  ]
)

追踪到401570，可以看到逻辑一大串。大致看下来就是用来做base64编码的。所以实际上需要找的是一串存好的base64编码。返回main追踪a5mc58bphliax7j得到db
'5Mc58bPHLiAx7J8ocJIlaVUxaJvMcoYMaoPMaOfg15c475tscHfM/8=='。又因为是自定义符集编码，所以再追踪aQvejafhmuyjbac得到'qvEJAfHmUYjBac+u8Ph5n9Od17FrICL/X0gVtM4Qk6T2z3wNSsyoebilxWKGZpRD'
