...
#show: book.with(title: "lab0_reuse")

= CTF101 短期课程 Lab0 Report
<ctf101-短期课程-lab0-report>
== 3240102120
<section>
本报告用于提交 CTF101 课程Lab
0。由于去年参加了校赛，虽然成绩较差，但是仍然希望通过本课程学习 CTF
相关知识。
...
\- 参数起名比较随意 - 语法不会特别规范 - 部分黑盒内容不了解 -
部分指令理解出现偏差
敬请谅解！！！如有任何需要改进的问题，烦请及时指出，我将及时修改！！！

!!! Node "一些代名" !!! Quote "常言道…" 不会打歌嘛学打歌，阿哥咋摆你咋摆
学习前辈经验，在CTF上面有我个人的一些代名，在我的报告中可能出现在举例子、实例展示等环节，以下是我常用的一些代名。这并非做记号，因此在此提前声明！
\- 2120（我的学号后四位） - AlabTNT - 41ebNTN - hjt - MoonCake\_\_MC -
ybmc - M00nC4k1
...
- `screen` : 创建新窗口，通常被我拿来挂一些后台程序，用法就是
  `screen -S <name>`来创建窗口、`screen -r <name>`
  attach一个窗口，`screen -list` 来列出全部已打开窗口。
- `ssh` :
  用来使用ssh进行远程连接或是拉取内容等等。ssh后面一般跟地址。比如
  `ssh root@alabtnt.com`
- `tail` : 展示一个文件的末尾内容。
- `ps` : 查看全部进程。
- `pwd` : 展示当前所在位置的绝对路径。
- `dir` : 显示当前文件夹全部内容，也可以用 `ls` 代替。例如
  `ls -a`可以展示详细文件。
- `cd` : `dir` 和 `pwd`
  太水了（虽然这个也很水）：进入一个文件夹。索引父级文件夹使用 `..` 例如
  `cd ../alabtntctf` 进入与当前所在位置同级的另一个文件夹。

连接到我的 Linux 机器：`wsl -d Debian`。或者连接到我的 Linux 服务器
`ssh root@******.***`
...
#figure(image("lab0_0.png", alt: "alt text"),
  caption: [
    alt text
  ]
)

题目描述有一个进程正在向bad.log里面持续写东西，关掉这个"磁盘爆爆器"，首先它是个进程，所以先`ps auxf`拿列表。然后扫了一下就能找到一个叫`badlog.py`的python进程，于是尝试kill一下。kill完check
solution就过了。

==== Challenge 2
<challenge-2>
Python我熟悉啊……Python我可太熟悉了（
...
#figure(image("lab0_1.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
首先观察题目，题目里面提示了要了解base相关内容，那理所应当的往base上面想。因为以=结尾，所以考虑Base64+。尝试了一遍后发现Base85解码后以==结尾。所以继续使用Base64解码。再度解码后又以===结尾，于是烘焙解码过程就是Base85-Base64-Base32，最终获得了这个flag。

==== Challenge 2
<challenge-2-1>
图片隐写，这个我曾经干过，在某个发布的工具包logo图片里面藏涩图（x）

flag：`AAA{gr3@t_J08!_1et'5_P1@y_m1SC_TOG3Th3R}`

hint给完的难度就低了很多。首先是LSB隐写，用在线工具Aperi'Solve自动分析，发现Red通道下可以找到图片中的前半段flag：

#figure(image("lab0_2.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
然后看到了比较特殊的`Nhe`, `tB`,
`H`，首先是这个`tB`，因为英文单词必须有元音（含半），而ta, te, ti, tu,
ty,
tw都不是单词，所以`B`只能代表o。而相应的`H`可能是a或i。又he结尾的三个英文单词的词只有she,
the有意义，前面的`A`已经代表t了，于是`N`只能代表s。
...
紧接着可以看到`Eo`，猜测是go。于是`E`=g。另外我在开发游戏谁是国王的时候，这个byd游戏最开始叫King
Assassin，当时就开玩笑说这是king
ass♂ass♂in，没想到我看到这个`HssHssDC`还真能让我想起来这个byd词，于是`HDC`就是ain。
...
`aOsolStelU`已经写脸上了，O=b S=u U=y, `abanPoneP`
是经典老四六级词abandoned, P=d, `MroRessional` 是 professional, M=p, R=f

\[A=t, F=h, G=e, B=o, N=s, E=g, H=a, D=i, C=n, I=w, J=l, K=r, L=v, O=b,
S=u, U=y, P=d, M=p, R=f\]
```xjbpd_6
...
这是个非常简单的RSA题，RSA原理这里不再赘述，因为给定了 \$ p \$ 和 \$ q
\$ ，所以直接上python脚本计算 \$ \(n)=(p-1)\*(q-1) \$ 和 \$ d= \$ 即可。
...
+ Line28使用`strlen(tmp->data)`参与计算，而`strlen()`的计算方法是#strong[直到空字符]，因此这里给入一个不以'\\0'结尾的内容就可以在30行的时候overflow

+ Line44声明err但未初始化值，而在Line55为err唯一的赋值语句，这导致如果Line53的条件不满足，那么err将以未定义的值返回。
...
==== Task3
<task3>
（因为不知道是否可以提交code.zip所以把nobug\_program.c贴到报告里好了）
...
首先测试发现输入1和2会返回两句不同的话，输入3以上就爆了，又说在table=flag和column=flag的地方，所以尝试注入select
flag from
flag，但是会被检测注入，尝试用/\*\*/代替空格也不奏效，然后发现用2|1这样的语句会返回结果，所以考虑用if语句和纯函数注入，方法是检测输入的值和表值的ascii是不是相等，相等就输出1，不然就输出2，这样输出1就代表正确，输出2就代表不正确。为了减少遍历次数，使用二分搜索。
...
#figure(image("reverseyitizuobuchulai.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("1726afb752ded5231e1c369bafb25f80.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("06206ec8ee738a2e981dcfb9071eedf8.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-1.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
