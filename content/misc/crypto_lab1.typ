...
#show: book.with(title: "crypto_lab1")
...
阅读完程序后可知加密方法为使用randrange生成不定长(15 \~ 29)的1 \~
96数字作为密钥串，然后按维吉尼亚方法对原字符串进行加密，区别在于本程序是进行了乘法运算，而空格位于index=0的位置，因此对于任何空格都不加密。
...
既然是乘法运算，紧跟着就会有下个突破口：密文列实际上是一个余数列。那么理论上来说，我们需要做的就是对1\*1
\~
96\*96里的所有数做质数分解，以此来确定每一个密文数可能对应的明文。又因为两个乘数都在1
\~ 96内，因此写一个这样的函数来试图反推密文：
...
#figure(image("image-12.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
感觉做到这工具函数差不多齐了，就是unlock\_1,2,3。unlock1因为发展原因被黑盒套起来了，所以只要知道某一列的数对应的字母就可以直接用unlock\_3拿密钥，更新rrkey然后让程序调用unlock\_2用密钥解这列其他数。循环直到解出29个数为止。这里推理过程有时候依靠灵感，所以尽可能简写，不再赘述。为了不再只适应345，以下对for
i in segments改进：
...
3行：空 Am 9 64 81 46 96 24
空，因为前面是the，所以考虑可能是American，暂时先不写。

6行：空 citize 65 8 空，猜测citizen,
虽然8还不确定，但是确定第九列65是n。

6行确定了第三行第九列是e，所以确定是American，于是直接解出10\~15列

18行：空 New 空 Yo 61 5 88 空，猜测是New
York,或.。先认定16,17列的61,5是rk

31行：空 physi 71 58 空，猜测是physics，解出18,19列的71,58是cs

1行：空 the 空 nineteen 57 24 空，猜测是二十世纪，进而推测前面的85
82是In，解出1,2,20,21列。但是似乎In解错了，退回-1

3行：空 langu 49 62 26 44 空猜测是language，解出22,23,24列

4行：空 begi 92 … 猜测是begin…，解出25列

6\~7行：United 空 43 6 54 16 93 s.。直接猜测 United
States，解出27,28,29,1,2列。（到这验证出前面那个In猜错了，实际是By）
...
#figure(image("image-13.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-14.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
原理如下： $ e d = k phi\(n\)+ 1 $ 假设已知的 \$ d \$ 是 \$ d\_0 \$ ，设
\$ s=p+q \$ ，就有 $ p e d = p k\(n - s + 1\)+ p $ 而根据
$ p s = p^2 + p q = p^2 + n $ 代入就可以消掉 \$ s \$ ，得到
$ p e d = k p n - k\(p^2 + n\)+ p k + p $ 而 $ d = d_0 + x $ ，这里 \$ x
\$ 是不知道的低位 \$ d \$ ，于是
$ p e d_0 + p e x = k p n + p k + p - k p^2 - k n $ 由于 \$ x \$
不知道且比较小，而且建立在已知高位爆破 \$ p \$
的算法基础上，所以可以直接忽略 \$ pex \$ 这一项，直接求解
$ p e d_0 = k p n + p k + p - k p^2 - k n $ 可以得到一个近似的 \$ p \$
，因为高位的 \$ d \$ 是准确的，所以这里 \$ p \$
的高位也是准确的。接下来再进行高位爆破 \$ p \$ 即可。

而高位爆破 \$ p \$ 的原理是，因为 \$ p | n \$ ，所以对于方程
$ x equiv - p_(h i g h)\(m o d p\) $ 来说，
$ x equiv - p_(h i g h)\(m o d n\) $
也能求得一个可能满足的小解。从而解得全部 \$ p \$ 。
