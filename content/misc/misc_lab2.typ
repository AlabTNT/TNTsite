...
#show: book.with(title: "misc_lab2")
...
#figure(image("image-20.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-21.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("songmingti2.jpg", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-23.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-24.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-25.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-26.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-27.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-28.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#box(image("nanami2.png"))

还没有flag，但是既然是对话框……是不是图片被截过？用hex把高度从60改成600：

#figure(image("image-29.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("nanami3.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-30.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
下一步是生成了一个spectrogram。经过我的不懈努力测试（比如我并不特别熟悉ndarray所以还转了int），发现这是个长32宽320的二维数组。结合前面的`num_freqs=32`，而且数了一下给的gif的条数正好是32，所以认为这个长是每一个频率（可能？）对应的每一帧的播放情况。

接下来往后是依托答辩，不得不说这个压行写的是真的好（？），最外层的gif\_data是`for frame in spectrogram.transpose()`这个生成器生成的，而`transpose()`结果是转置，也就是每个frame都是一个长32的列，按我这个是有320帧。那么就是说接下来的操作是把这320帧计算然后写成gif。
...
接下来看array()生成的内容，里面是两个list套一块了，第一个list生成器是`for freq in range(num_freqs * 2 + 1)`，也就是freq在0\~64之间遍历，第二个list的生成器是`for i in reversed(range(min_db, max_db + 1, quantize))`，i是31\~-60两步一步进，也就是31,29,…,-59。生成器里面的内容是主要的，生成了color\_pixel或while\_pixel，如果freq是奇数且i比`frame[freq//2]`的四舍五入要小，前者是为了让条形之间有空隙，比如freq作为4的时候，5和3之间就有一条纯白，以此来分割5和3。后者是数据，生成的时候如果i比指定数据小了就都生成为带色，否则生成白色，也就是说，生成出来的要作为array()的数列是一个xy坐标图，它按从下到上、从左到右的顺序绘制了一个点状图。如果把点扩展成1\*1的阴影格（比如程序里写的纯蓝色），那么就成了一个条形图。
...
#figure(image("image-31.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
那么可以听到flag-1是Never gonna give you
up（你被骗了），flag-2是一剪梅（卤蛋.mp4）
...
#figure(image("image-32.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
哇，输出了一坨，挨个分别看，大致是input是一堆字符，input\_id是一堆int，power是一堆float。总之先把这一堆存成文件：
...
#box(image("image-34.png", alt: "alt text"))
#box(image("image-33.png", alt: "alt text"))
#box(image("image-35.png", alt: "alt text"))

可以看到，input内容是a-z,0-9,\_{}，input\_id内容是0\~26，每个数字39（=26+10+3）个，共27\*39=1053行，power就是1053行数据，每行有100个数据。合理猜测input\_id告诉我们flag长度为27，power告诉我们正确的input，input是编码集。
...
#figure(image("image-36.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-37.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-38.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
