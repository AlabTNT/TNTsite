...
\ -\> 虽然我人已经来到了现场，如下图所示： \
#box(image("c806efa8d4100d8dc34830eef22c8f51.png", alt: "alt text"))

== Task 1
<task-1>
+ 使用指令`nslookup www.zju.edu.cn`，得到结果如下：
...
+ 实际上我想的是直接用自己的公网服务器进行nslookup，不过既然题目是这样出的，那就代表应该用`nslookup www.zju.edu.cn 8.8.8.8`或者114.114.114.114。这里使用8.8.8.8
]
...
#figure(
  align(center)[#table(
    ...
    table.header([接口路径], [方法], [参数],),
    table.hline(),
    [/jwglxt/cxdy/xscjcx\_cxXscjIndex.html?gnmkdm=N5083&layout=default&su=3240102120], [GET], [gnmkdm=N5083&layout=default&su=3240102120],
    [/jwglxt/xtgl/index\_cxMyCosJxpj.html?gnmkdm=N5083&su=3240102120], [POST], [gnmkdm=N5083&su=3240102120],
    [/jwglxt/cxdy/xscjcx\_cxXscjIndex.html?doType=query&gnmkdm=N5083&su=3240102120], [POST], [?doType=query&gnmkdm=N5083&su=3240102120],
  )]
  , kind: table
  )
...
接下来编写python。作为noob of
python，只需花费114514时间就可以写出这样的一个request：
...
那么根据这个原理，只需要先发送一个包，然后把G修改成全POST请求，在末尾添加一个类似Content:
的内容，接下来发送的包就会被认为是Content内容，也就是评论区的内容，这样受害者再发送包时服务器就会把包的全部内容当作Content传入评论区。主动抓取评论功能的POST包，可以得到如下内容：
