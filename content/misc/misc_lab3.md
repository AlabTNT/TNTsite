...
根据challenge.py的内容，使用resnet18模型，然后给了一个transform，选了一个显卡。打开本地的img.png，之后循环接受输入，直到某一次单行输入END，把之前所有行数据按顺序拼接成一个字符串，认为这是一个图片的base64。得到这两个图片后，先是比较max_diff，也就是两个图间每个像素的差值绝对值，有大于6的就退出，没有的话，继续用刚才的transform处理读入的图片，这里transform的变换是随机的，随机裁剪+随机水平翻转，然后给了一大堆参数变换。最后调用ai模型检测这个图片能否识别为原来的img.png的107模型（我也不知道107模型是什么，应该是个magic number）且概率在95%以上。这个检测要执行五次，每次都要在95%以上才能最终输出flag。
