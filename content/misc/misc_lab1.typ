...
#show: book.with(title: "misc_lab1")
...
交一下发现不是flag，所以还得解。原文里面有提示说"原理类似于贝斯五十X贝斯二十X码"，所以考虑一下Base58，但是因为原文里面有o和l，估计base58也要败下阵来，（这里卡了一个下午+一晚上，而且听说去年只有十几个人做出NATO26，有1点草单

早上起来重新考虑了一下贝斯五八，结合题目叫NATO26，所以考虑是不是BASE28.
所以就应该是把a#sub[z作为0]25，然后按位相加得到一个数，这个数转hex之后两位一截转utf-8。所以编写了如下脚本执行这个任务：
...
运行即可获得flag （虽然只用了一行但是非常的臭）

（后面和其他几个人讨论了一下才知道这玩意居然出题的时候就出全损题，那有点坏了）

=== Task2.2.1
<task2.2.1>
参考了#link("https://zhuanlan.zhihu.com/p/453675608")[直呼赚烂]
...
#figure(image("image.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
+ 已知两个设备的名字是相同的，它们的名字是什么？(如果设备名中有空格则用\_替代，例如Name
  1.0写为Name\_1.0)
+ suspicious\_traffic.pcap流量所对应的应用程序是什么？(英文名词单词，如application)
+ 目标设备的分布式设备管理组件版本号是什么？(如1.1.1)
+ 通信过程使用的软总线版本号是多少？(如999)

第一个问题：只需要使用Ctrl+F按字节流搜索devicename即可，可以找到devicename是：OpenHarmony
3.2

#figure(image("image-2.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-3.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-4.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-5.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
=== Task 4
<task-4>
校巴上的crack\_zju\_wlan。首先下载附件得到一个.img文件。用file查看发现是一个linux下的ext2挂载映像，所以使用mount把它挂载到一个文件夹下。打开文件夹就能获得crack\_zju-01.cap

hint里给定了一个xxx-ng用于分析网络流量，去bing上把hint原封不动进行搜索可以得到aircrack-ng是提示的全文。所以下载一个aircrack-ng文件，然后用这个文件打开crack\_zju-01.cap，但是结果是只找到了180个IV，而分析需要至少5000个IV。考虑是不是这一个.cap不够，可能还存了其他cap。

回去用ls
\-a查看隐藏文件，找到了一个.password.txt.swp，于是用`vim -r .password.txt.swp`恢复文件得到了一个password.txt。那么使用`aircrack-ng crack_zju-01.cap -w password.txt`，终端就有输出`KEY FOUND! [ 0YcWPeLMBp ]`。所以flag就是`AAA{0YcWPeLMBp}`
