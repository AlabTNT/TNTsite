...
#figure(image("image-7.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
得到user的密码`I_am_very_very_strong_password!!`，长度为31。而源码中会把0处理，因此总长度为32。于是nc之后输入user、32、上述密码即可得到flag1：`flag1: AAA{Oh_D1rTy_sta`

#figure(image("image-8.png", alt: "alt text"),
  caption: [
    alt text
  ]
)

再破admin的密码。admin的密码会读入password\_verify然后验证password\_verify和password是否相同。之后的wrong\_password区会输出用户输入的password。但是是使用%s方式输出，因此只要不到\\0就会持续输出，而username、password和password\_verify是相邻地址，因此只需要构造一个非\\0结尾的密码即可溢出到password\_verify，另外还需要使username填满32位，所以username要构造admin+AAAAA…到32，密码则填充32个B：
...
#figure(image("image-9.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-10.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
=== Task3
<task3>
这个题比较综合了，首先是要完成delegate
task，看源码是要直接写进内存的，所以要提前编译成字节传入，这个不得不借助python了：
...
#figure(image("image-11.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
#figure(image("image-15.png", alt: "alt text"),
  caption: [
    alt text
  ]
)
...
题目大概意思就是要stack
overflow，但是跳转地址是0x20000+MRND是环境变量，好在MRND就八个，可以爆，所以写这么个程序
