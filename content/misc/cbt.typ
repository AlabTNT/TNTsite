...
+ B。pip是python的库管理器，可以用于安装、卸载第三方库。常用的命令是`pip install <包名>`,
  `pip uninstall <包名>`,
...
+ C。A中10%2==0为True，中间是or连接，故为True；B中'b'\<'c'字符串的比较是True，详见知识点；C中'0'的结果是True（详见上一条知识点），not
  True就是False；D中结果为2j，complex数不为0即为True。
]

!!! Note "相关知识点：逻辑判断" 逻辑判断的核心就是and
or和not。and和or优先级低于not。所有表达式先计算not（有括号除外），然后从左到右计算and和or。python是"短路判断"，即判断到有False
and …的时候，结果立即返回False；判断到有True or
…的时候，结果立即返回True。
...
+ B。详见上一题解析。在count==2时，输出"2
  "，count+=1（count现在==3），然后跳转回while，count\<5成立，于是进入循环：if
  count==3成立，于是进入if分支：break，跳出循环。

+ A。（卧槽这个题你们也考）B生成的是0到49的列表`[0,1,2,...,49]`，C生成的是0\~200中的偶数列表`[0,2,4,...198]`，D生成的是True和False的列表，偶数位是True，奇数位是False`[True,False,True,...False]`
]
...
`Python     def add(a:int,b:int):         print(a,"+",b,"=",a+b)     c=10     d=20     add(c,d)     add(b=c,a=d)`
...
+ D。groupA=groupA+(5,6)=(1,2,3,4)+(5,6)=(1,2,3,4,5,6)；groupB.update(groupA)
  \=\> groupB=set(groupA)=set((1,2,3,4,5,6))={1,2,3,4,5,6}，故选D。
]
...
+ B。本题考查局部变量和全局变量。在函数内的变量作为局部变量只能在函数内使用，且优先级高于全局变量。本题中x=10定义的是全局变量，x=20定义的是my\_function()的局部变量，接下来的`print(x,end=' ')`调用的是局部变量。之后的`print(x)`调用的是全局变量。
...
!!! Danger "这个题是错题！"
这个题是错题，正确答案是0，如果认为是最后一行的输出可以选10，实际输出结果是第一行为0，第二行为10。可见外院计算机老师的不严谨程度，尤其是Python语言。
...
+ F。is可能更偏向自然语言，例如`[1,2,3] is list`返回True。is判断的是二者是否为同一类，==严格判断二者是否相等。尤其在自定义class时，如果没有定义\_\_eq\_\_函数，两个实例用==比较时比较的是两个实例的存储地址而非它们的各项值。
...
+ F。这样设置只能让ASCII的负号改成Unicode的负号，并不能解决中文乱码问题。正确做法是指定字体，例如`plt.rcParams['font.sans-serif']=['SimHei']`指定为微软雅黑字体。

== 填空题
<填空题>
+ 5.0-8\*2\*\*3%5/\/3-True=5.0-8\*8%5/\/3-True=5.0-64%5/\/3-True=5.0-4/\/3-True=5.0-1-True=4.0-True=4.0-1=3.0。
...
#block[
#set enum(numbering: "1.", start: 4)
+ \(year%400==0) or (year%4==0 and
  year%100!=0)。闰年是每四年一闰，整百年时整400才能闰。
...
+ popitem()。字典的用法。可以参考#link("https://www.runoob.com/python3/python3-dictionary.html")[这篇教程]最后的部分

+ ndarray。Numpy中的数列一般都是ndarray。

+ 文本文件。文本文件是可读的文件，二进制文件是编码好的用于交予计算机执行的文件。

+ end='\;'。print函数的用法之一。还有类似的sep=""。end用于指定print结尾的内容，默认是换行符""；sep用于指定print多参数之间的分割，默认是空格"
  "。例如print(1,2)实际输出是"1
  2"，print(1,2,sep=",",end="\;")实际输出是"1,2;"
...
+ 元组。`return a,b,c,...`返回多个元素时，会自动使用元组打包。

+ lst\[start\],lst\[end\]=lst\[end\],lst\[start\]。这里是python的一个语法糖，即多变量赋值。`a,b,c,...=x,y,z,...`相当于`a=x,b=y,c=z,...`，但还是有一些细微的区别。当x,y,z…是变量时，赋值语句首先取临时变量表出来将x,y,z…一一写入，然后再一一赋给a,b,c…。这也就导致了`lst[start],lst[end]=lst[end],lst[start]`是交换值，而`lst[start]=lst[end],lst[end]=lst[start]`是覆盖值（后者损失了`lst[start]`原来的值）

+ if
  n%i==0。这里判断从2开始到根号n结束的所有整数中有没有数可以整除n（即除后余数等于0）
]

== 附录1：常见保留字
<附录1常见保留字>
False, None, True, and, as, break, class, continue, def, del, elif,
else, except, finally, for, from, if, import, in, is, not, or, pass,
